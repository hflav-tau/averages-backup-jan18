************************************************************************
*
*     "CHI2_SYM_CIRC" combination routine for COMBOS program
*
...
*     -- modifications to use new routine CIRCULARITY
*     -- use of array NEEDED
*     -- add printout at initialisaton
*     Version 1.21, January 21, 1997:
*     -- bug fixed: column I and row I of matrix EXOUT are now set to 0 
*                   if I is not an internal MINUIT parameter
*     Version 1.30, January 27, 1997:
*     -- extra argument SPECIAL in FCN_CHI2_SYM_CIRC_INIT
*     -- allow to solve circularity with external constraints on chid and fs
*     -- improved printout at finalization
*     Version 1.40, February 6, 1997:
*     -- implement protection to prevent MPAR from exceeding MAXPAR
*     -- parameter MAXPAR increased from 20 to 30
*     Version 1.50, February 12, 1997:
*     -- add output argument CORR in call to routine CIRCULARITY
*     -- print full error/correlation matrix on all quantities used to solve
*        the circularity
*     Version 2.00, February 20, 1997:
*     -- parameter MAXPAR increased from 30 to 50
*     -- first argument removed in call to MINUIT_CHI2
*     -- two first arguments removed in call to PREPARE_CHI2
*     -- two first arguments removed in entry FCN_CHI2_SYM_CIRC
*     -- write output on logical unit LUNIT (no output if LUNIT is zero or less)
*     Version 2.10, February 27, 1997:
*     -- modified printout (replaced "world average" with "extended average")
*     Version 2.13, March 10, 1997:
*     -- print PARAMETER logical lines for possible use in subsequent jobs
...
************************************************************************
*
      SUBROUTINE CHI2_SYM_CIRC(CVAL,ERR2P,ERR2N,CL,IERR)
*     ==================================================
*
*     Olivier Schneider, CERN/PPE-ALE
*     December 11, 1996
...
*
      ...
      INTEGER IERR
      EXTERNAL FCN_CHI2_SYM_CIRC_INIT,FCN_CHI2_SYM_CIRC
*
      CALL MINUIT_CHI2(FCN_CHI2_SYM_CIRC_INIT,FCN_CHI2_SYM_CIRC,
     &                 CVAL,ERR2P,ERR2N,CL,IERR)
      END
*
************************************************************************
*
      SUBROUTINE FCN_CHI2_SYM_CIRC(NPAR,D,F,X,IFLAG,FUNC)
*     ==================================================
*
...
*
      ...
      INTEGER NPAR,IFLAG
*
*     Argument of entry FCN_CHI2_SYM_CIRC_INIT
*
...
*
      IF(NOT_READY) THEN
        PRINT * ,'FCN_CHI2_SYM_CIRC: not initialized !!!'
        STOP 566
      ENDIF
*
...
*
      IF(IFLAG.EQ.2) THEN
        PRINT * ,'FCN_CHI2_SYM_CIRC: cannot compute derivatives'
        STOP 669
      ENDIF
*
...
*
      IF(NPAR.NE.LPAR) THEN 
        PRINT * ,'FCN_CHI2_SYM_CIRC: problem NPAR,LPAR = ',NPAR,LPAR
        STOP 543
      ENDIF
*
...
*
      CALL CIRCULARITY(INDX,XIN,EXIN,XOUT,EXOUT,CORR)
*
...
*
      WRITE(LUNIT,5000) 
 5000 FORMAT(/,1X,'Final CHI2_SYM_CIRC result:')
      WRITE(LUNIT,5001) 
...
*
 ...
 6002 FORMAT(' PARAMETER ',A,3(2X,F10.5,SP),' ! CHI2_SYM_CIRC output')
      DO I=1,NNEWPAR
     ...
     &   CHOLDPAR(I),XOUT(I),SQRT(EXOUT(I,I)),-SQRT(EXOUT(I,I))
      ...
      RETURN
*
      ENTRY FCN_CHI2_SYM_CIRC_INIT(SPECIAL,IERR)
*     ==========================================
*

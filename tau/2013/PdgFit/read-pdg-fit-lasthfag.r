...
rc = lapply(split(df.nodes.fit[in.pdg.not.in.hfag, ], 1:nrow(df.nodes.fit[in.pdg.not.in.hfag, ])),
  function(cmp.fit) {
...
##--- compare only results also in HFAG
df.nodes.fit = df.nodes.fit[!in.pdg.not.in.hfag, ]

...
\mysubsubsubsection{$\Bz \to D^{\pm}\pi^{\mp}, D^{*\pm}\pi^{\mp}, D^{\pm}\rho^{\mp}$
}
...
% Fig.~\ref{fig:cp_uta:ckmfitter_sin2beta} displays the constraints 
...
%       \includegraphics{figures/cp_uta/ckmfitter_sin2beta}
%     }
%     \resizebox{0.95\textwidth}{!}{\hspace{1cm}
%       \includegraphics{figures/cp_uta/utfit_sin2beta}
%     }
%   \end{center}
%   \vspace{-0.5cm}
%   \caption{
%     Typical Standard Model constraints on the $(\rhobar,\etabar)$ plane,
%     from (top)~\cite{Charles:2004jd} and 
%     (bottom)~\cite{Bona:2005vz}.
%   }
%   \label{fig:cp_uta:ckmfitter_sin2beta}
% \end{figure}
...
%%%
%%%%%%%%
\mysubsubsection{Time-dependent analysis of $\Bs \to J/\psi \phi$
}
\label{sec:cp_uta:ccs:jpsiphi}

As described in Sec.~\ref{sec:cp_uta:notations:Bs},
time-dependent analysis of $\Bs \to J/\psi \phi$ probes the 
$\CP$ violating phase of $\Bs$--$\Bsb$ oscillations, $\phi_s$.
Within the Standard Model, this parameter is predicted to be small.\footnote{
   We make the approximation $\phi_s = ¡Ý2 \beta_s$, 
...
The combination of results is performed by the HFAG Lifetimes and Oscillations group, see Sec.~\ref{sec:life_mix}.
...
%% From the results in Tab.~\ref{tab:cp_uta:pipik0_tddp},
...
Direct \CP violation parameters for each of the contributing helicity
...
In addition to the $\KS\pi^0\gamma$ decay, both \babar\ and \belle have presented results using $\KS\eta\gamma$ and $\KS\rho\gamma$, and \belle\ have in addition presented results using $\KS\phi\gamma$~\cite{Sahoo:2011zd}.
...
\mysubsubsection{$D$ decays to multiparticle self-conjugate final states}

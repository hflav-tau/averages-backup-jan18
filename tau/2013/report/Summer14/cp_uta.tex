...
\newpage %% ugly hack to prevent pagebreak just after subsubsubsection heading
...
corrected statistically. % treatment is necessary to correct for this bias.
...
%%%
%%%%%%%%
\mysubsubsection{Time-dependent analysis of $\Bs$ decays through the $b \to c\bar{c}s$ transition
}
...
Within the Standard Model, this parameter is predicted to be small.\footnote{
   ...
   (see Sec.~\ref{sec:cp_uta:introduction}). 
   ...
   such that the approximation remains valid.
}
...
\mysubsubsection{Time-dependent $\CP$ asymmetries: $b \to c\bar{u}d$ decays to
  \CP eigenstates
}
\label{sec:cp_uta:cud_beta:cp}
...
than for $b \to c\bar{c}s$ decays such as $\B \to \jpsi \KS$.
...
In the latter two modes, the daughter decay $D^* \to D\pi^0$ is used.
...
and for both combined, taking into account the different $\CP$ factors
(denoted $D^{(*)}_{\CP} h^0$).
The results are summarised in Table~\ref{tab:cp_uta:cud_cp_beta}.

\input{tables/cp_uta/dcph0.tex}

\mysubsubsection{Time-dependent Dalitz plot analyses of $b \to c\bar{u}d$ decays
}
\label{sec:cp_uta:cud_beta:dalitz}

When multibody $D$ decays, such as $D \to \KS\pi^+\pi^-$ are used, 
a time-dependent analysis of the Dalitz plot of the neutral $D$ decay 
allows for a direct determination of the weak phase: $2\beta$. 
...
Due to the suppression of the Standard Model amplitude, contributions of additional diagrams from physics beyond the Standard Model,
with heavy virtual particles in the penguin loops, may have observable effects.
...
For the $\Bz \to K^+K^-\Kz$ decay, both \babar\ and \belle\ measure the \CP
...
In addition to the $\KS\pi^0\gamma$ decay, both \babar\ and \belle\ have presented results using $\KS\eta\gamma$ and $\KS\rho\gamma$, and \belle\ has in addition presented results using $\KS\phi\gamma$.
...
\mysubsubsection{Constraints on $\alpha \equiv \phi_2$}
...
helps resolve some of the multiple solutions, 
...
At present we make no attempt to provide an HFAG average for $\alpha \equiv \phi_2$.
...
Non-$\CP$ eigenstates such as $D^\mp\pi^\pm$, $D^{*\mp}\pi^\pm$ and $D^\mp\rho^\pm$ can be produced 
...
For each mode, $D\pi$, $D^*\pi$ and $D\rho$, 
...
\mysubsubsection{Time-dependent $\CP$ asymmetries in $\Bz \to D^\mp \KS \pi^\pm$}

Time-dependent analyses of transitions such as $\Bz \to D^\mp \KS \pi^\pm$ can
...
Compared to the situation for $\Bz \to D^{(*)\mp} \pi^\pm$ discussed above (Sec.~\ref{sec:cp_uta:cud}), the larger value of the ratio $R$ of the magnitudes of the suppressed and favoured amplitudes allows it to be determined from the data.  
...
Various methods using different $\DorDstar$ final states have been used.
...
For ADS analyses, all of \babar, \belle, CDF and LHCb have studied the modes 

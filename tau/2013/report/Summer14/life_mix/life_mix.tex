...
Hadronic events produced in $e^+e^-$ collisions at the \Upsfive (also known as
...
However, the assumption $\fnBfive=0$ is known to be invalid since the observation of
...
%%% LHCb report a $p_{\rm T}$ dependence for \Lb production relative to
%%% ${\Bu}$ and ${\Bd}$.  The reported $p_{\rm T}$ dependence enhances the 
%%% number of \Lb baryons observed at low-$p_{\rm T}$ compared with the
%%% LEP results.}
...
\particle{B_s^{**0} \to B^{(*)}K} also contribute to the \Bu and \Bd rates, 
...
\Figure{rlb_comb} shows the ratio $R_{\Lb}$ as a function of 
...
of $R_{\Lb}$ combined with the constraint from \Eq{constraints} implies
...
Two fits are performed. % similar to the result for $R_{\Lb}$ above.r
...
% Bs lifetime, Bc lifetime, \Lb and \b-baryon lifetimes, theoretical predictions
...
the Standard Model.\footnote{
  ...
  For reasons of symmetry in \Eqss{dm}{dg}, 
...
A large number of time-dependent \Bd--\Bdbar oscillation analyses
...
%OS% Their average is compatible with the recent and more precise measurements 
%OS% from the asymmetric \B factories and the LHCb experiment.
...
Note that \Eq{Vtd_over_Vts} assumes that \dms and \dmd only receive 
...
of \Eq{dmd_over_dms} to set limits on 
new physics effects. 
...
Note that the asymmetry of time-dependent decay rates in \Eq{ASL} is time-independent.
...
\begin{equation} 
%TimGershon% \frac{N(\hbox{\Bd}(t) \to D^{(*)-}\mu^+\nu_{\mu}X)-N(\hbox{\Bdbar}(t) \to D^{(*)+}\mu^-\bar{\nu}_{\mu}X)}%
%TimGershon% {N(\hbox{\Bd}(t) \to D^{(*)-}\mu^+\nu_{\mu}X)+N(\hbox{\Bdbar}(t) \to D^{(*)+}\mu^-\bar{\nu}_{\mu}X)} =
% The above two lines were introduced instead of the two lines below, but I think this was wrong (i.e. the two lines below are correct, since the initial state is untagged):
\frac{N(D^{(*)-}\mu^+\nu_{\mu}X)-N(D^{(*)+}\mu^-\bar{\nu}_{\mu}X)}%
{N(D^{(*)-}\mu^+\nu_{\mu}X)+N(D^{(*)+}\mu^-\bar{\nu}_{\mu}X)} =
...
\Table{qoverp} summarizes the different measurements: 
...
% \babar \cite{Aubert:2006nf,*Aubert:2002mn_mod_cont}  & dileptons % superseded by \cite{Lees:2014qma,*Lees:2014qma_cont}
%                              & \multicolumn{3}{c}{} % superseded by \cite{Lees:2014qma,*Lees:2014qma_cont}
%                              & 0.9992 & 0.0027 & 0.0019 \\  % superseded by \cite{Lees:2014qma,*Lees:2014qma_cont}
...
\labe{Dzero_ASLDS}
\end{eqnarray}
where the fractions $f'$ have been defined in \Eq{chibar}.

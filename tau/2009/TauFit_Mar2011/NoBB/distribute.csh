...
for dirname in \
  ...
  if [ "${dirname}" == "NoBB_lepcorr_addratio_2" ] ; then
    echo '\noindent Using inputs from: the non-B-Factory measurements of single quantities and ratios (eg. $\mathrm{B_\mu/B_e}$) and including leptonic correlations only.' > what_is_this.tex
  fi
  ...
  if [ "${dirname}" == "NoBB_addpre-bbrcorr_addratio_4" ] ; then
    echo '\noindent Using inputs from: all the non-B-Factory measurements including all the correlations.' > what_is_this.tex
  fi
  ./summary0.csh

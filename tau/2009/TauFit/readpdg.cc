...
void print_node_def(int nnode, char** a_nodename, string* nodetitle,  vector<string> nodegammaname,
  ...
  for (p=0;p<2;++p) {
    for (inode=0;inode<nnode;++inode) {
      if (p==1&&(node_num_npar[inode]+node_den_npar[inode])==1&&inode!=102) continue;  // derived node [inode = 102 is special case : Gamma96 = (1/1.699387) * Gamma801]
...
int main(int argc, char* argv[]){
  ...
  //
  // Correlation between G(tau- --> eta K- pi0 nu(tau)) and G(tau- --> eta pi- K0bar nu(tau))
  //
  cout << Form("Corr between %s and %s = %10.6f\n",
